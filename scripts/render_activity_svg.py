...
def snake_frames(route, food, initial_length=8):
    """Keep the tail still on eating steps, growing one cell per food square."""
    body=list(range(initial_length-1,-1,-1))
    frames=[body.copy()]
    for head in range(initial_length,len(route)):
        body.insert(0,head)
        if route[head] not in food:
            body.pop()
...
def render(data):
    ...
    food={(week,row) for week,row,day in cells if day['level']}
    route=[(x,0) for x in range(-8,0)]
    route += [(x,y) for y in range(7) for x in (range(weeks) if y%2==0 else range(weeks-1,-1,-1))]
    route += [(x,6) for x in range(weeks,weeks+8+len(food))]
    frames=snake_frames(route,food)
    reached={point:i-7 for i,point in enumerate(route)}
    frame=.04
    begin=.7
    duration=(len(frames)-1)*frame
    restore=begin+duration+.3
    def xy(point):
        x,y=point
    ...
    parts=[f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}" font-family="Consolas,Menlo,monospace" font-size="12">',
        ...
        '<desc>A pale blue snake grows by one segment for each contribution square eaten. Contribution colors then return and remain visible. Click the image in the profile to inspect GitHub contribution details.</desc>',
    ...
    # Each segment follows simulated body positions, pausing when food is eaten.
    key_times=';'.join(f'{i/(len(frames)-1):.7f}' for i in range(len(frames)))
    for segment in range(len(frames[-1])-1,-1,-1):
        born=next(i for i,body in enumerate(frames) if len(body)>segment)
        origin=frames[born][segment]
        points=';'.join(f'{(body[segment] if len(body)>segment else origin)/(len(route)-1):.7f}' for body in frames)
        fill=['#d9f4ff','#b5e8fc','#8ed8f8','#74c7e9','#60b1d4','#4892b4','#35728e','#28566d'][min(segment,7)]
        parts.append(f'<g opacity="0"><set attributeName="opacity" to="1" begin="{begin+born*frame:.3f}s" fill="freeze"/><animate attributeName="opacity" from="1" to="0" begin="{begin+duration:.3f}s" dur=".1s" fill="freeze"/><animateMotion path="{motion}" keyPoints="{points}" keyTimes="{key_times}" rotate="auto" begin="{begin:.3f}s" dur="{duration:.3f}s" calcMode="linear" fill="freeze"/><rect x="-6.5" y="-6.5" width="13" height="13" rx="4" fill="{fill}"/>')
        if segment==0:
